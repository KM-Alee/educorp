...
from app.relay import PublishingOutboxRelay
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    relay: PublishingOutboxRelay | None = None
    try:
        relay = PublishingOutboxRelay(session_factory)
        await relay.start()
    except Exception as exc:
        logger.warning("Publishing outbox relay unavailable", exc_info=exc)
        relay = None
...
def create_app() -> FastAPI:
    """Application factory."""
    app = FastAPI(
        title="EduCorp Publishing Service",
        version="0.1.0",
        lifespan=lifespan,
    )
    ...
    app.include_router(v1_router, prefix="/api/v1/publishing")
